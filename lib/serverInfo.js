...
async function detectProxy(externalIP, ipGeo) {
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // LAYER 4 — VPN/Tunnel, MTU, Latency, Hardcoded IPs, All IP Sources
  // ═══════════════════════════════════════════════════════════════════════════

  result.details.vpnTunnel = {
    tunnelInterfaces: [],
    mtuAnalysis: [],
    vpnProcesses: [],
    vpnConfigs: [],
  };
  result.details.hardcodedIPs = {
    etcHosts: null,
    networkInterfaces: null,
    netplan: null,
    systemdNetworkd: null,
    cloudInit: null,
    dhcpLeases: null,
    ifcfgScripts: null,
  };
  result.details.allIPSources = {};
  result.details.latencyCheck = null;

  // ── 18. VPN / Tunnel interface detection ──
  const ifaces = os.networkInterfaces();
  const tunnelPrefixes = ["tun", "tap", "wg", "ppp", "gre", "vti", "ipip", "sit", "wireguard", "tailscale", "utun", "ipsec", "vxlan", "geneve"];
  for (const [name, addrs] of Object.entries(ifaces)) {
    const nameLower = name.toLowerCase();
    for (const prefix of tunnelPrefixes) {
      if (nameLower.startsWith(prefix) || nameLower.includes(prefix)) {
        const ifaceIPs = addrs.map((a) => a.address);
        result.details.vpnTunnel.tunnelInterfaces.push({ name, ips: ifaceIPs });
        result.indicators.push(`Tunnel/VPN interface detected: ${name} (${ifaceIPs.join(", ")})`);
        result.isProxied = true;
        break;
      }
    }
  ...
  // Also check ip link for interfaces not visible to Node
  const ipLink = safeExec("ip -o link show 2>/dev/null");
  if (ipLink) {
    for (const line of ipLink.split("\n")) {
      const nameMatch = line.match(/^\d+:\s+(\S+?)[@:]/);
      if (!nameMatch) continue;
      const ifName = nameMatch[1];
      const ifLower = ifName.toLowerCase();
      for (const prefix of tunnelPrefixes) {
        if (ifLower.startsWith(prefix) || ifLower.includes(prefix)) {
          const already = result.details.vpnTunnel.tunnelInterfaces.some((t) => t.name === ifName);
          if (!already) {
            result.details.vpnTunnel.tunnelInterfaces.push({ name: ifName, ips: [], source: "ip-link" });
            result.indicators.push(`Tunnel/VPN interface in ip link: ${ifName}`);
            result.isProxied = true;
          }
          break;
        }
  ...
  // ── 19. MTU analysis — tunnels have smaller MTU than 1500 ──
  for (const [name, addrs] of Object.entries(ifaces)) {
    // Get MTU via ip link
    const mtuInfo = safeExec(`cat /sys/class/net/${name}/mtu 2>/dev/null`);
    const mtu = mtuInfo ? parseInt(mtuInfo) : null;
    if (mtu) {
      result.details.vpnTunnel.mtuAnalysis.push({ interface: name, mtu });
      if (mtu < 1500 && mtu > 0) {
        // Check if this is the default route interface
        const defaultIface = result.details.networkStack.routingTable
          ? (result.details.networkStack.routingTable.match(/default.*dev\s+(\S+)/) || [])[1]
          : null;
        if (defaultIface === name || !defaultIface) {
          result.indicators.push(`Interface ${name} has MTU ${mtu} (< 1500) — likely tunneled (WireGuard=1420, OpenVPN≈1400)`);
          result.isProxied = true;
        }
      }
    }
  ...
  // ── 20. VPN process detection ──
  const vpnProcesses = [
    { name: "wireguard", cmd: "pgrep -f wireguard 2>/dev/null || ls /sys/class/net/wg* 2>/dev/null" },
    { name: "openvpn", cmd: "pgrep -x openvpn 2>/dev/null" },
    { name: "tailscale", cmd: "pgrep -f tailscaled 2>/dev/null" },
    { name: "ipsec/strongswan", cmd: "pgrep -f 'charon|strongswan|ipsec' 2>/dev/null" },
    { name: "zerotier", cmd: "pgrep -f zerotier 2>/dev/null" },
    { name: "nebula", cmd: "pgrep -x nebula 2>/dev/null" },
    { name: "tinc", cmd: "pgrep -x tincd 2>/dev/null" },
    { name: "pptp", cmd: "pgrep -x pptpd 2>/dev/null" },
    { name: "l2tp", cmd: "pgrep -f xl2tpd 2>/dev/null" },
    { name: "cloudflared", cmd: "pgrep -f cloudflared 2>/dev/null" },
    { name: "bore", cmd: "pgrep -f bore 2>/dev/null" },
    { name: "frp", cmd: "pgrep -f 'frps|frpc' 2>/dev/null" },
    { name: "ssh-tunnel", cmd: "pgrep -f 'ssh.*-[DLR]' 2>/dev/null" },
  ];
  for (const vpn of vpnProcesses) {
    if (safeExec(vpn.cmd)) {
      result.details.vpnTunnel.vpnProcesses.push(vpn.name);
      result.indicators.push(`VPN/tunnel process running: ${vpn.name}`);
      result.isProxied = true;
    }
  }

  // ── 21. VPN config file detection ──
  const vpnConfigPaths = [
    { name: "WireGuard", path: "/etc/wireguard/" },
    { name: "OpenVPN", path: "/etc/openvpn/" },
    { name: "Tailscale", path: "/var/lib/tailscale/" },
    { name: "StrongSwan", path: "/etc/ipsec.conf" },
    { name: "StrongSwan", path: "/etc/strongswan/" },
    { name: "ZeroTier", path: "/var/lib/zerotier-one/" },
    { name: "Nebula", path: "/etc/nebula/" },
    { name: "Cloudflared", path: "/etc/cloudflared/" },
  ];
  for (const vpnConf of vpnConfigPaths) {
    if (fs.existsSync(vpnConf.path)) {
      result.details.vpnTunnel.vpnConfigs.push(vpnConf);
      result.indicators.push(`VPN config found: ${vpnConf.name} at ${vpnConf.path}`);
      result.isProxied = true;
    }
  }

  // WireGuard kernel module check
  const wgModule = safeExec("lsmod 2>/dev/null | grep wireguard");
  if (wgModule) {
    result.indicators.push("WireGuard kernel module loaded");
    result.isProxied = true;
  }

  // ── 22. Hardcoded IPs — check all network config files ──

  // /etc/hosts — hardcoded IP mappings
  const etcHosts = safeReadFile("/etc/hosts");
  if (etcHosts) {
    const hostEntries = etcHosts.split("\n")
      .filter((l) => l.trim() && !l.startsWith("#"))
      .map((l) => {
        const parts = l.trim().split(/\s+/);
        return { ip: parts[0], hostnames: parts.slice(1) };
      });
    result.details.hardcodedIPs.etcHosts = hostEntries;
    // Check if our external IP or hostname is hardcoded
    const hostname = os.hostname();
    for (const entry of hostEntries) {
      if (entry.ip === externalIP) {
        result.indicators.push(`External IP ${externalIP} found hardcoded in /etc/hosts → ${entry.hostnames.join(", ")}`);
      }
      if (entry.hostnames.includes(hostname) && entry.ip !== "127.0.0.1" && entry.ip !== "::1") {
        result.indicators.push(`Hostname "${hostname}" is mapped to ${entry.ip} in /etc/hosts`);
      }
    }
  }

  // /etc/network/interfaces (Debian/Ubuntu classic)
  const netInterfaces = safeReadFile("/etc/network/interfaces");
  if (netInterfaces) {
    result.details.hardcodedIPs.networkInterfaces = netInterfaces;
    const staticBlocks = netInterfaces.match(/iface\s+\S+\s+inet\s+static[\s\S]*?(?=iface|\Z)/gi);
    if (staticBlocks) {
      for (const block of staticBlocks) {
        const addrMatch = block.match(/address\s+(\S+)/);
        if (addrMatch) {
          result.indicators.push(`Static IP configured in /etc/network/interfaces: ${addrMatch[1]}`);
        }
      }
    }
  }
  ...
  if (fs.existsSync(netplanDir)) {
    try {
      const files = fs.readdirSync(netplanDir).filter((f) => f.endsWith(".yaml") || f.endsWith(".yml"));
      const configs = [];
      for (const file of files) {
        const content = safeReadFile(`${netplanDir}${file}`);
        if (content) {
          configs.push({ file, content });
          // Look for static addresses
          const addrMatches = content.match(/addresses:\s*\n(\s+-\s+\S+)+/g);
          if (addrMatches) {
            result.indicators.push(`Static IPs in netplan ${file}: ${addrMatches.join("; ").replace(/\n/g, " ")}`);
          }
        }
      }
      if (configs.length > 0) result.details.hardcodedIPs.netplan = configs;
    } catch { /* skip */ }
  }

  // systemd-networkd
  const networkdDir = "/etc/systemd/network/";
  if (fs.existsSync(networkdDir)) {
    try {
      const files = fs.readdirSync(networkdDir).filter((f) => f.endsWith(".network"));
      const configs = [];
      for (const file of files) {
        const content = safeReadFile(`${networkdDir}${file}`);
        if (content) {
          configs.push({ file, content });
          const addrMatch = content.match(/Address\s*=\s*(\S+)/gi);
          if (addrMatch) {
            result.indicators.push(`Static IPs in systemd-networkd ${file}: ${addrMatch.join(", ")}`);
          }
        }
      }
      if (configs.length > 0) result.details.hardcodedIPs.systemdNetworkd = configs;
    } catch { /* skip */ }
  }

  // cloud-init network config
  const cloudInitNet = safeReadFile("/etc/cloud/cloud.cfg.d/50-curtin-networking.cfg") ||
                       safeReadFile("/var/lib/cloud/instance/network-config.json") ||
                       safeExec("cat /var/lib/cloud/instance/obj.pkl 2>/dev/null | strings | grep -E '\\b\\d+\\.\\d+\\.\\d+\\.\\d+\\b' | head -5");
  if (cloudInitNet) {
    result.details.hardcodedIPs.cloudInit = cloudInitNet;
    const ipMatches = cloudInitNet.match(/\b\d+\.\d+\.\d+\.\d+\b/g);
    if (ipMatches) {
      const uniqueCloudIPs = [...new Set(ipMatches)].filter((ip) => ip !== "0.0.0.0" && ip !== "255.255.255.255");
      if (uniqueCloudIPs.length > 0) {
        result.indicators.push(`IPs in cloud-init config: ${uniqueCloudIPs.join(", ")}`);
      }
    }
  }

  // DHCP leases — the IP given by the DHCP server
  const dhcpLease = safeExec("cat /var/lib/dhcp/dhclient*.leases 2>/dev/null | tail -20") ||
                    safeExec("cat /var/lib/dhclient/dhclient*.leases 2>/dev/null | tail -20") ||
                    safeExec("cat /var/lib/NetworkManager/*.lease 2>/dev/null | tail -20");
  if (dhcpLease) {
    result.details.hardcodedIPs.dhcpLeases = dhcpLease;
    const leaseIP = dhcpLease.match(/fixed-address\s+(\S+)/);
    if (leaseIP) {
      result.indicators.push(`DHCP lease IP: ${leaseIP[1]}`);
    }
  }

  // ifcfg scripts (RHEL/CentOS/Fedora)
  const ifcfgDir = "/etc/sysconfig/network-scripts/";
  if (fs.existsSync(ifcfgDir)) {
    try {
      const files = fs.readdirSync(ifcfgDir).filter((f) => f.startsWith("ifcfg-"));
      const configs = [];
      for (const file of files) {
        const content = safeReadFile(`${ifcfgDir}${file}`);
        if (content) {
          configs.push({ file, content });
          const ipMatch = content.match(/IPADDR\s*=\s*["']?(\S+)["']?/i);
          if (ipMatch) {
            result.indicators.push(`Static IP in ${file}: ${ipMatch[1]}`);
          }
        }
      }
      if (configs.length > 0) result.details.hardcodedIPs.ifcfgScripts = configs;
    } catch { /* skip */ }
  }

  // ── 23. Exhaustive IP collection — every possible source ──
  const allIPs = {};

  // a) Node.js os.networkInterfaces()
  const nodeIPs = [];
  for (const [ifName, addrs] of Object.entries(ifaces)) {
    for (const addr of addrs) {
      nodeIPs.push({ interface: ifName, ip: addr.address, family: addr.family, internal: addr.internal });
    }
  }
  allIPs.nodeInterfaces = nodeIPs;

  // b) ip addr show
  const ipAddrShow = safeExec("ip -o addr show 2>/dev/null");
  if (ipAddrShow) {
    const parsedAddrs = [];
    for (const line of ipAddrShow.split("\n").filter(Boolean)) {
      const match = line.match(/^\d+:\s+(\S+)\s+inet6?\s+(\S+)/);
      if (match) parsedAddrs.push({ interface: match[1], cidr: match[2] });
    }
    allIPs.ipAddrShow = parsedAddrs;
  }

  // c) hostname -I (all local IPs)
  const hostnameIPs = safeExec("hostname -I 2>/dev/null");
  if (hostnameIPs) allIPs.hostnameI = hostnameIPs.trim().split(/\s+/);

  // d) hostname -i
  const hostnameSmallI = safeExec("hostname -i 2>/dev/null");
  if (hostnameSmallI) allIPs.hostnameSmalli = hostnameSmallI.trim();

  // e) External IP services (already collected)
  allIPs.externalServices = result.details.multiServiceIPs;
  allIPs.primaryExternal = externalIP;

  // f) Cloud metadata IPs (already collected)
  allIPs.cloudMetadata = {
    privateIP: result.details.cloudMetadata.realIP,
    publicIP: result.details.cloudMetadata.elasticIP,
  };

  // g) DNS resolution of own hostname
  const ownHostname = os.hostname();
  const dnsResolve = safeExec(`getent hosts ${ownHostname} 2>/dev/null`) ||
                     safeExec(`dig +short ${ownHostname} 2>/dev/null`);
  if (dnsResolve) allIPs.dnsHostnameResolve = dnsResolve.trim();

  // h) STUN — query a STUN server to get reflexive IP (what the internet sees)
  // Using a DNS-based method as a fallback
  const stunIP = safeExec("dig +short myip.opendns.com @resolver1.opendns.com 2>/dev/null");
  if (stunIP) {
    allIPs.stunDns = stunIP.trim();
    if (externalIP && stunIP.trim() !== externalIP) {
      result.indicators.push(`DNS-based IP check (OpenDNS) sees ${stunIP.trim()}, primary sees ${externalIP}`);
      result.isProxied = true;
    }
  }

  // i) Cloudflare trace IP (already collected)
  if (result.details.dnsLeakCheck?.ip) {
    allIPs.cloudflareTrace = result.details.dnsLeakCheck.ip;
  }

  result.details.allIPSources = allIPs;

  // ── 24. Latency-based geo verification ──
  // Ping a known server in the geo-claimed region and see if RTT matches
  if (ipGeo?.countryCode) {
    const latencyTargets = {
      US: { host: "us-east.speed.cloudflare.com", expected: 50 },
      DE: { host: "eu-central.speed.cloudflare.com", expected: 30 },
      GB: { host: "eu-west.speed.cloudflare.com", expected: 30 },
      JP: { host: "asia-east.speed.cloudflare.com", expected: 50 },
      SG: { host: "asia-southeast.speed.cloudflare.com", expected: 50 },
      AU: { host: "au-east.speed.cloudflare.com", expected: 50 },
    };

    // Generic latency test — measure RTT to a well-known endpoint
    ...
    try {
      ...
      // Also try an endpoint geographically near the claimed IP
      let regionalRtt = null;
      const target = latencyTargets[ipGeo.countryCode];
      if (target) {
        const pingResult = safeExec(`ping -c 1 -W 2 ${target.host} 2>/dev/null`);
        if (pingResult) {
          const rttMatch = pingResult.match(/time[=<](\d+\.?\d*)/);
          if (rttMatch) regionalRtt = parseFloat(rttMatch[1]);
        }
      }

      result.details.latencyCheck = {
        cloudflareRtt: rtt,
        regionalTarget: target?.host || null,
        regionalRtt,
        ipCountry: ipGeo.countryCode,
      };

      // If RTT to Cloudflare is very high (>300ms) but IP claims nearby, suspicious
      if (rtt > 300 && ["US", "DE", "GB", "NL", "FR"].includes(ipGeo.countryCode)) {
        result.indicators.push(`High RTT to Cloudflare (${rtt}ms) despite IP claiming ${ipGeo.countryCode} — possible distant proxy`);
        result.isProxied = true;
      }

      // If regional ping is very high compared to expected
      if (regionalRtt && target && regionalRtt > target.expected * 5) {
        result.indicators.push(`Regional ping to ${target.host}: ${regionalRtt}ms (expected ~${target.expected}ms for ${ipGeo.countryCode}) — location may not match`);
        result.isProxied = true;
      }
    } catch { /* skip */ }
  }

  // ── 25. IP rule / policy routing ──
  const ipRules = safeExec("ip rule show 2>/dev/null");
  if (ipRules) {
    const ruleLines = ipRules.split("\n").filter(Boolean);
    // More than the default 3 rules (0: local, 32766: main, 32767: default) = policy routing
    if (ruleLines.length > 3) {
      result.indicators.push(`Policy routing detected: ${ruleLines.length} ip rules (default is 3) — traffic may be selectively routed`);
      result.isProxied = true;
    }
  }
