...
async function detectProxy(externalIP, ipGeo) {
  ...
  // 1. Reverse DNS — proxy/CDN IPs often resolve to their infra hostnames
  if (externalIP) {
  ...
  // 2. Check known CDN/proxy org names in the IP geo data
  if (ipGeo?.org) {
  ...
  // 3. Compare server timezone with IP geo location
  ...
  // 4. Query multiple IP services and compare
  const ipServices = [
  ...
  // 5. Proxy-related environment variables
  const proxyEnvKeys = [
